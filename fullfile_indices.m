%'fullfile_indices': creates a file name from root name, NomType, indices and file extension. 
%------------------------------------------------------------------------
% filename=fullfile_indices(RootFile,FileExt,NomType,i1,i2,j1,j2)
%------------------------------------------------------------------------           
...
function filename=fullfile_indices(RootFile,FileExt,NomType,i1,i2,j1,j2)
    
%% display help and test function in the absence of input argument
if ~exist('RootFile','var')
    help fullfile_indices;
    test;
    return
end
...
filename=[RootFile sep1 i1_str sep2 i2_str sep3 j1_str sep4 j2_str];
...
for ilist=1:numel(FileName_list)
    [~,~,RootFile,i1,i2,j1,j2,FileExt,NomType]=...
        fileparts_uvmat(FileName_list{1,ilist});
    FileName_list{2,ilist}=['NomType=' NomType];
    FileName_list{3,ilist}=['i+1,j+1->' fullfile_indices(RootFile,FileExt,NomType,i1+1,i2+1,j1+1,j2+1)];
