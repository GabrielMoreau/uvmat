...
if isfield(DataIn,'Action') && isfield(DataIn.Action,'RUN') && isequal(DataIn.Action.RUN,0)
    ...
    if OK==1
        ...
        prompt = {'nbre of points for the sliding window'};% titles of the edit boxes
        %default input:
...
for pos=1:size(Var,2)
    ...
    if numel(ind_good)>WindowLength
        ...
        cospecmean=cospec+cospecmean;
    end
end
specmean=specmean/NbPos;
cospecmean=cospecmean/NbPos;
...
hold on
loglog(freq_max*(1:length(cospecmean))/length(cospecmean),cospecmean,'r')
hold off
