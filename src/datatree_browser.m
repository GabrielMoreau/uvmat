...
% Last Modified by GUIDE v2.5 29-Jul-2012 08:49:20
...
function datatree_browser_OpeningFcn(hObject, eventdata, handles, InputName)
%------------------------------------------------------------------------
...
if exist('InputName','var')
    if isdir(InputName)% opening by uvmat/MenuSetProject
        set(handles.SourceDir,'String',InputName)% the input directory is considered as a project
        ...
        if exist(InputDir,'dir')
            ...
            if isfield(s,'SourceDir')
                ...
                set(handles.LinkDir,'String',InputDir)
                set(handles.LinkDir,'Visible','on')
            else
                set(handles.SourceDir,'String',InputDir)
                set(handles.LinkDir,'Visible','off')
...
%delete(handles.datatree_browser)

%------------------------------------------------------------------------
% --- Executes on button press in MarkupDir: create a xml file to mark SourceDir as project source
function MarkupDir_Callback(hObject, eventdata, handles)
%------------------------------------------------------------------------
SourceDir=get(handles.SourceDir,'String');
t=xmltree;
t=set(t,1,'name','Project');
try
save(t,[SourceDir '.xml'])
catch ME
    msgbox_uvmat('ERROR',ME.message)
end

%------------------------------------------------------------------------
% --- Executes on button press in UpdateLink: create a link dir or update it if it exists
function UpdateLink_Callback(hObject, eventdata, handles)
%------------------------------------------------------------------------
[SourcePath,ProjectName]=fileparts(get(handles.SourceDir,'String'));
LinkDir=get(handles.LinkDir,'String');
if isempty(LinkDir)||strcmp(get(handles.LinkDir,'Visible'),'off')
    LinkRoot=uigetdir(LinkDir,'pick up the path to the link directory'); %file browser
    if isempty(LinkRoot),return,end
    LinkDir=fullfile(LinkRoot,[ProjectName '.link']);
    set(handles.LinkDir,'Visible','on')
    set(handles.LinkDir,'String',LinkDir)
end
...
LinkDirXml=fullfile(fileparts(LinkDir),[ProjectName '.link.xml']);
if ~exist(LinkDirXml,'file')
    LinkDoc.SourceDir=get(handles.SourceDir,'String');
    t=struct2xml(LinkDoc);
    t=set(t,1,'name','Project');
    save(t,LinkDirXml)
    set(handles.UpdateLink,'String','update_link')
end

FillExperiments(handles)

%------------------------------------------------------------------------
% --- Fill the column 'Experiments' with the list of detected directries
function FillExperiments(handles)
%------------------------------------------------------------------------    
SourceDir=get(handles.SourceDir,'String');
LinkDir='';
...
idir=0;
ExpName=cell(length(hdir),1);
for ilist=1:length(hdir)
    if hdir(ilist).isdir
        dirname=hdir(ilist).name;
        if ~isequal(dirname(1),'.')&&~isequal(dirname(1),'0')% do not list directories beginning by '0'
            idir=idir+1;
...
set(handles.ListExperiments,'String',[{'*'};ExpName(1:idir)])
...
for iexp=1:numel(ListExperiments)
    ...
    for ilist=1:length(hdir)
        ...
        if ~isequal(Device(1),'.')
            ...
            check_list=strcmp(Device,ListDevices);
...
%------------------------------------------------------------------------
% --- Executes on selection in column ListDevices
...
ListExperiments=get(handles.ListExperiments,'String');
Experiment=ListExperiments{list_val};
RootPath='';
if strcmp(get(handles.LinkDir,'Visible'),'on')
    RootPath=get(handles.LinkDir,'String');
end
if isempty(RootPath)
RootPath=get(handles.SourceDir,'String');
end
...
% end
% set(handles.ListExperiments,'Value',1)
% ListExperiments_Callback(handles)%update the overview of the experiment directories
% DataviewData=get(handles.datatree_browser,'UserData');
...
set(handles.ListExperiments,'Value',1)
ListDataDir(hObject, eventdata, handles)%update the overview of the experiment directories
DataviewData=get(handles.datatree_browser,'UserData')
List=DataviewData.List;
Currentpath=get(handles.SourceDir,'String');
[Currentpath,Campaign,DirExt]=fileparts(Currentpath);
...
uiresume(handles.datatree_browser);
% 
...
% --- Executes when user attempts to close datatree_browser.
function datatree_browser_CloseRequestFcn(hObject, eventdata, handles)
% if isequal(get(handles.datatree_browser, 'waitstatus'), 'waiting')
